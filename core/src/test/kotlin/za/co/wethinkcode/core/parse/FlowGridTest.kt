...
class FlowGridTest {
    ...
    @Test
    fun `empty`() {
        assertThat(grid[0, 0].type).isEqualTo(CellType.Nothing)
    ...
    @Test
    fun `existing cell`() {
        grid[0, 0] = FlowCell(NO_DETAIL, CellType.CommitLeft)
        assertThat(grid[0, 0].type).isEqualTo(CellType.CommitLeft)
    }
    ...
    fun `width and height change even with sparse`() {
        grid[3, 7] = FlowCell(NO_DETAIL, CellType.CommitLeft)
    ...
    fun `less than width and height don't change width and height`() {
        ...
        assertThat(grid.width).isEqualTo(4)
        assertThat(grid.height).isEqualTo(8)
        grid[2, 6] = FlowCell(NO_DETAIL, CellType.CommitLeft)
        assertThat(grid.width).isEqualTo(4)
        assertThat(grid.height).isEqualTo(8)
    }
}